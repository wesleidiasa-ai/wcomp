...
- **Menu por etapa do processo de compras**: o menu lateral reflete o funil (Solicitações → Aprovações → Cotações → Pedidos → Recebimentos), cada etapa com sua própria fila de trabalho, indicadores específicos e badge de contagem (Aprovações/Cotações) — em vez de uma lista única com filtro de status. "Solicitações" é a visão ampla (todos os status); "Todos os pedidos" continua existindo à parte, com busca instantânea (número, item, solicitante, fornecedor, observação) e filtros completos por status, setor, solicitante, fornecedor, período, valor e urgência
- **Tabela de pedidos** (compartilhada entre todas as telas acima): prioridade com cor/ícone; valor em destaque acima de um limiar; progresso mini (stepper compacto) e ícone de categoria por setor (heurística por palavra-chave, não uma taxonomia real); alertas de "parado há X dias" e "fornecedor atrasado"; linha inteira clicável; ação rápida de duplicar pedido; exportação em CSV (na tela "Todos os pedidos")
